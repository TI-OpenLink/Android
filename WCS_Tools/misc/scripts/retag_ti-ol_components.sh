...
cd workspace/wlan
repo forall -c 'git tag -f -a ${NEW_TAG_NAME} ${CURRENT_TAG_NAME} -m \"${NEW_TAG_NAME}\"'
...
repo forall -c 'if [ ${REPO_REMOTE} == "TI-OpenLink" ] ; then git tag -f -a ${NEW_TAG_NAME} ${CURRENT_TAG_NAME} -m \"${NEW_TAG_NAME}\" ; fi'
...
repo forall -c 'if [ ${REPO_REMOTE} == "TI-OpenLink-secured" ] ; then git tag -f -a ${NEW_TAG_NAME} ${CURRENT_TAG_NAME} -m \"${NEW_TAG_NAME}\" ; fi'
