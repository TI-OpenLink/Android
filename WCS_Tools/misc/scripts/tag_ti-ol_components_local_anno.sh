...
cd workspace/wlan
repo forall -c 'echo "updating project ${REPO_PROJECT} with new tag ${NEW_TAG_NAME}" ; git tag -f -a ${NEW_TAG_NAME} -m ${NEW_TAG_NAME}'
cd -
...
repo forall -c 'if [ ${REPO_REMOTE} == "TI-OpenLink" ] ; then \
	...
	git tag -f -a ${NEW_TAG_NAME} -m ${NEW_TAG_NAME} ; \
fi'
